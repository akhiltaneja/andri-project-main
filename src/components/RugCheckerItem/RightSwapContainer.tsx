...
const initialWaringMessage: WaringMessage[] = [
    {
        message: "Duplicated code",
        tooltip: "The contract source is not unique, there have been similar contracts deployed prior to this one.",
    }
];
...
const RightSwapContainer = ({ tokenAddress }: ReportProps) => {
    ...
    const [WarningMessageArray, setWarningMessageArray] = useState(initialWaringMessage);
    ...
    useEffect(() => {
        ...
        const onGetMessages = async () => {
            try {
                const api_url = `https://rugchecker.windswap.finance/rug/${tokenAddress}`
                const response = await fetch(api_url)
                const res = await response.json()
                console.log('new_data = ', res)
                console.log('new_data_messages = ', res.messages.warning);
                setWarningMessageArray(res.messages.warning);
            } catch (error) {
                console.error('Unable to fetch data:', error)
            }
        }
    ...
    return (<div>
        ...
        {isDark === true && <div className="rug-page-right-bottom-card-container-style">
            ...
            {WarningMessageArray && WarningMessageArray.map(item => {
                return (<div className="rug-checker-message-container-style">
                    <p className="rug-checker-message-header-text-style">{item.message}</p>
                    <p>{item.tooltip}</p>
                </div>)
            })}
